...
pub async fn create_dealer(
    ...
    let result = sqlx::query(
        "INSERT INTO dealers (name, kana, address, phone, fax, email) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(name)
    .bind(&kana)
    .bind(&address)
...
pub async fn update_dealer(
    ...
    let old_name: Option<String> =
        sqlx::query_scalar("SELECT name FROM dealers WHERE id = ? AND deleted_at IS NULL")
            .bind(id)
    ...
    match result {
        ...
        Ok(_) => {
            ...
            .execute(&mut *transaction)
            .await?;
            sqlx::query("UPDATE dealer_contacts SET dealer_name = ? WHERE dealer_name = ?")
                .bind(name)
                .bind(&old_name)
                .execute(&mut *transaction)
                .await?;
...
pub async fn create_dealer_contact(
    ...
    let dealer_exists: Option<i64> =
        sqlx::query_scalar("SELECT id FROM dealers WHERE name = ? AND deleted_at IS NULL")
            .bind(&dealer_name)
...
pub async fn delete_dealer_contact(
    ...
    authenticate_admin(&state, &headers).await?;
    let rows = sqlx::query("UPDATE dealer_contacts SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL")
        .bind(id)
        .execute(&state.pool)
        .await?
        .rows_affected();
...
pub async fn permanently_delete_dealer_contact(
    ...
    authenticate_admin(&state, &headers).await?;
    let result = sqlx::query("DELETE FROM dealer_contacts WHERE id = ? AND deleted_at IS NOT NULL")
        .bind(id)
        .execute(&state.pool)
        .await?;
