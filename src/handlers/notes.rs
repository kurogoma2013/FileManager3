...
pub async fn create_project_note(
    ...
    sqlx::query("INSERT INTO project_notes (project_id, content, created_by) VALUES (?, ?, ?)")
        .bind(project_id)
        .bind(content)
        .bind(&username)
        .execute(&state.pool)
...
pub async fn create_dealer_note(
    ...
    sqlx::query("INSERT INTO dealer_notes (dealer_id, content, created_by) VALUES (?, ?, ?)")
        .bind(dealer_id)
        .bind(content)
        .bind(&username)
        .execute(&state.pool)
...
pub async fn delete_project_note(
    ...
    let result = sqlx::query("UPDATE project_notes SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND project_id = ? AND deleted_at IS NULL")
        .bind(note_id)
        .bind(project_id)
        .execute(&state.pool)
        .await?;
...
pub async fn restore_project_note(
    ...
    let result = sqlx::query("UPDATE project_notes SET deleted_at = NULL WHERE id = ? AND project_id = ? AND deleted_at IS NOT NULL")
        .bind(note_id)
        .bind(project_id)
        .execute(&state.pool)
        .await?;
...
pub async fn delete_dealer_note(
    ...
    let result = sqlx::query("UPDATE dealer_notes SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND dealer_id = ? AND deleted_at IS NULL")
        .bind(note_id)
        .bind(dealer_id)
        .execute(&state.pool)
        .await?;
...
pub async fn restore_dealer_note(
    ...
    let result = sqlx::query("UPDATE dealer_notes SET deleted_at = NULL WHERE id = ? AND dealer_id = ? AND deleted_at IS NOT NULL")
        .bind(note_id)
        .bind(dealer_id)
        .execute(&state.pool)
        .await?;
